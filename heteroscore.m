function patients = heteroscore(patients)
...
num_pixels = zeros(n_patients, n_slices);


for p = 1:n_patients

    hetmaps = patients(p).hetero_images;
    n_slices = size(patients(p).hetero_images, 3);
%     hetscore = zeros(n_slices, 1);
    ...
    for n=1:n_slices
        ...
        total_pixels = total_pixels + num_pixels(p, n);
...
%         title(['het. hist. img#',sprintf('%d',n)])
        vecsmax(n) = max(max(t0(:, p, n)/num_pixels(p, n), vecsmax(n)));
...
%         plot(loglabvec, t0_log/num_pixels, 'k-');
        vlogmax(n) = max(max(t0_log(:, p, n)/num_pixels(p, n), vlogmax(n)));
    end
...
for p = 1:n_patients
    ...
    for n = 1:n_slices
        ...
        hold on;
        subplot(4,3,mod(n-1,3)+1+mod(floor((n-1)/3),2)*6)
        plot(labelvec, t0(1:end-1, p, n)/num_pixels(p, n), line);
        title(['het. hist. img#',sprintf('%d',n)])
        ...
        subplot(4,3,mod(n-1,3)+1+mod(floor((n-1)/3),2)*6)
        title(['log, het. hist. img#',sprintf('%d',n)])
        plot(loglabvec, t0_log(1:end-1, p, n)/num_pixels(p, n), line);
    end
end
