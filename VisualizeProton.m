function VisualizeProton(handles)
...
ysize1=size(imresize(handles.patient(pat_index).body(:, :, 1),scale),1)
xsize1=size(imresize(handles.patient(pat_index).body(:, :, 1),scale),2)
matrix=zeros(ysize1,xsize1);
   multi=round(aspect*(ysize1/numslices));
      ...
      %ysize=size(handles.patient(pat_index).body(:, :, i),1);
      %xsize=size(handles.patient(pat_index).body(:, :, i),2);
      %totalsize=xsize*ysize/4;
      matrixtemp=imresize(handles.patient(pat_index).body(:, :, i),scale);
%matrixtemp =  reshape(handles.patient(pat_index).body(:, :, i),4,totalsize);
